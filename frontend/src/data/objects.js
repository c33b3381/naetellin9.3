/**
 * World Objects Database - All placeable object types for World Builder (F1)
 * Used by: WorldEditor, GameWorld (createWorldAsset)
 * 
 * Categories:
 * - Nature: Trees, bushes, flowers, rocks
 * - Rocks & Minerals: Stone, ores, crystals
 * - Buildings: Houses, towers, castles
 * - Walls & Fences: Barriers, gates
 * - Props & Decor: Furniture, decorations
 * - Lighting: Lamps, torches, braziers
 * - NPCs: Villagers, guards, merchants
 * - Vendor NPCs: Shopkeepers
 * - Monsters: Enemy spawn points
 * - Animals: Passive creatures
 * - Magical Items: Portals, crystals, runes
 * - Treasure & Loot: Chests, gold piles
 * - Crafting Stations: Forges, anvils, workbenches
 * - Furniture: Beds, tables, chairs
 * - Market & Trade: Stalls, carts
 * - Dungeon & Prison: Cages, chains, torture devices
 * - Military & Siege: Catapults, barricades
 * - Agriculture: Farms, silos, mills
 * - Maritime & Ships: Docks, boats, anchors
 */
...
import { 
  ...
  Shovel, Pickaxe, Key, DoorOpen, Target, Plus, Eye
} from 'lucide-react';
...
// Helper to get all object types as flat array
export const getAllObjectTypes = () => {
  const types = [];
  for (const [catKey, category] of Object.entries(OBJECT_CATEGORIES)) {
...
export const getCategoryByKey = (key) => OBJECT_CATEGORIES[key] || null;

// Helper to get object type by full key
export const getObjectType = (categoryKey, typeKey) => {
  const category = OBJECT_CATEGORIES[categoryKey];
  return category?.types?.[typeKey] || null;
};

export default OBJECT_CATEGORIES;
